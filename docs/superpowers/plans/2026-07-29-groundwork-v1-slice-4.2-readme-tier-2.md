...
- **Still open for the maintainer:** `gh issue close 10`; Codex's 3.1 finding 1 (should an absent `layers:` key ERROR); the health-metrics v2 candidate; the 3.3 hook-set call; three Slice 1.5d-ii deferrals; the `SKIP_RELPATHS` sign-off; the standing re-review rule; the `Motion: assist` reading; #21's `since:` retrofit; whether groundwork dogfoods its own hook set.
