...
> that installs rule 9. Plan-less work uses this location
> (`docs/superpowers/reviews/<branch>.md`, `/` written as `-`). Rule 9 states what must be
> committed before merge and how the log is appended; it is the normative text and this
> header does not restate it. **This log's own addition, which rule 9 does not require:**
> each finding is marked CONFIRMED (the reviewer verified it against a source) or
> PLAUSIBLE (reasoned, unverified). Whether that becomes part of rule 9 is a maintainer
