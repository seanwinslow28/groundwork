...
- `scripts/validate.py` — `diff_base_findings`, plus `_base_is_ancestor`,
  ...
  **not** remove them from `gov_roots` — round 2 measured why. `main()` runs the contract first, ahead
...
- `tests/test_validate.py` — `TestDiffBaseContract`. Each half has a paired control that
  breaks the thing it guards. `round-05.md` carries the mutation table as it stands, after
  round 5 measured two mutations that the round-04 table had claimed to cover and did not;
  the earlier entries carry the repairs they made, and
...
rule 5. It is **not a regression**: the fixture in
`test_deleting_the_marker_under_a_pre_marker_base_says_nothing` returns no finding from any
stateful pass on this branch, and returns none on `main` at `ea05b28` either. And **no check
reading only the base tree and the working tree can reach it** — with the marker in neither,
nothing tells it apart from an ungoverned repository, which the engine's own pin-less root is.
That limit is the two trees, not the git repository: round 5 pointed out that walking the
commits between base and HEAD could see a marker added and then deleted, which is a route
nobody here has explored. Issue #40 carries it. Rule 9's three rejection grounds are a closed
list and none fits, so this is recorded as open rather than dressed as something else.
...
| `python3 -m unittest discover -s tests -q` | OK, 824 tests, skipped=1 | OK, **846 tests**, skipped=1 |
