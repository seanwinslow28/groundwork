...
| The --diff base contract: a base holding each governed root's pin and each interview state's manifest, and standing in this branch's history; the tripwire skips a root the base predates; a marker the commits between base and HEAD added that neither tree now holds (18, 9, 40) | diff_base_findings | ERROR on a base missing a governed root's pin or an interview state's manifest, ERROR on a groundwork.pin or 00-manifest.md that a git log of base..HEAD reports as added and that neither the base tree nor the working tree holds (see docs/known-limitations.md for what that walk does not see), WARN on a base that is not an ancestor of HEAD and on an ancestry question git cannot answer |
